...
export class Fragola {
    ...
    public createMessageRouter(router: messageRouterMap): MessageRouter {
        ...
        for (const key in router) {
            let textRouterForAgent = structuredClone(textRouter);
            delete textRouterForAgent[router[key].agent?.name!];
            console.log(textRouterForAgent);
        }
...
export class Run {
    ...
    constructor(agentName: string, private params: Omit<ChatCompletionCreateParamsBase, "messages" | "tools">, private project: Fragola.Project, private sdk: OpenAI) {
        ...
        this.updateXML(() => ({
            ...
            systemPrompt: {
                ...
                Fragola instructions are considered 'extra' because they are not written by the user himself but injected at runtime by the library.`
                }
            }
            }
        }));
    ...
    [FragolaFriend] = {
        updateRunConfig: this.updateRunConfig.bind(this),
        updateSystemPrompt: this.updateSystemPrompt.bind(this),
